...
// You can also check multiple resource permissions at the same time
const canCreateProjectAndRevokeSession =
  await authClient.admin.checkRolePermission({
    permissions: {
      user: ["delete"],
      session: ["revoke"],
    },
...
<Callout type="warning">
  Any role that isn't in the `adminRoles` list, even if they have the
  permission, will not be considered an admin.
</Callout>
